...
class MainScreen(Screen):
    ...
    def _make_text_input(self, hint_text="", input_filter=None, height=CONTROL_HEIGHT, font_size=sp(17), **kwargs):
        ti = TextInput(
            ...
            disabled_foreground_color=COLOR_TEXT_SECONDARY,
            cursor_color=COLOR_PRIMARY,
            hint_text_color=COLOR_TEXT_SECONDARY,
            selection_color=(COLOR_PRIMARY[0], COLOR_PRIMARY[1], COLOR_PRIMARY[2], 0.30),
        ...
        with ti.canvas.after:
            Color(*COLOR_BORDER)
            ti.theme_border = Line(
                rounded_rectangle=(ti.x, ti.y, ti.width, ti.height, INPUT_RADIUS),
                width=dp(1)
            )

        def update_border(instance, value):
            instance.theme_border.rounded_rectangle = (
                instance.x, instance.y, instance.width, instance.height, INPUT_RADIUS
            )
    ...
    def delete_records(self, instance):
        ...
        close_btn.bind(on_press=popup.dismiss)
        popup.bind(on_dismiss=self._on_delete_records_popup_dismiss)
        self._refresh_delete_records_grid()
        popup.open()

    def _refresh_delete_records_grid(self):
        grid = self._delete_records_grid
        if grid is None:
            return
    ...
    def _on_delete_records_popup_dismiss(self, popup):
        if self._delete_records_popup is popup:
            self._delete_records_popup = None
            self._delete_records_grid = None

    def delete_single_record(self, record):
        record_index = next((i for i, item in enumerate(self.records) if item is record), None)
        ...
        if self.records:
            self._refresh_delete_records_grid()
        elif self._delete_records_popup is not None:
            self._delete_records_popup.dismiss()
    ...
    def clear_all_records(self, instance):
        def do_clear(btn):
            ...
            self.save_data()
            self.update_monthly_expense()
            if self._delete_records_popup is not None:
                self._delete_records_popup.dismiss()
